...
class RoIAttentionObjectToObjectConvFCBBoxHead(ConvFCBBoxHead):
    ...
    def __init__(self,
        ...
        # self.v_fc = nn.Linear(_in_channels, _in_channels)
        self.v_conv = nn.Conv2d(self.in_channels, self.in_channels, 3, padding = 1)
    ...
    def init_weights(self):
        ...
        # conv layers are already initialized by ConvModule
        for module_list in [self.q_fc, self.k_fc, self.v_conv]:
            for m in module_list.modules():
                if isinstance(m, nn.Linear):
                    nn.init.xavier_uniform_(m.weight)
                    nn.init.constant_(m.bias, 0)
    ...
    def forward(self, x):
        ...
        V = self.v_conv(x).reshape(-1, C*H*W)  # (BS*num_rois, attention_hidden_channels)
